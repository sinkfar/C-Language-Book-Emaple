...
void BookTopicProgramme()
{
	...
	printf("30.题目(15403)：看书！！！\n");
	printf("\n");

	printf("31.题目(15401)：看书！！\n");
	printf("\n");

	printf("32.题目(18101)：看书！！！\n");
	printf("\n");

	printf("33.题目(18202)：看书！！！\n");
	printf("\n");

	printf("34.题目(18203)：看书！！！\n");
	printf("\n");

	printf("35.题目(18204)：看书！！！\n");
	printf("\n");

	printf("36.题目(18205)：看书！！！\n");
	printf("\n");

	printf("37.题目(18206)：看书！！！\n");
	printf("\n");
}
...
int Book29(void)
{
	...
	int Input[10] = {54,87,66,22,11,43,68,0,29,78};
	int* p,*max,*min;
	int i,q1,q2;

	for (i = 0; i < 10; i++)printf(" %d ", Input[i]);
	printf("\n");

	max = Input+9;
	min = Input;

	for (p=Input;p<Input+10;p++)
	{
		if (*p > *Input || *p == *max)
		{
			q1 = *p;
...
int Book32(void)
{
	...
	for (i = 0; i < 5; i++)
	{
		open = student[i].math + student[i].physics + student[i].language;
		printf("%s %d %d %d %d\n", student[i].name, student[i].math, student[i].physics, student[i].language, open);
	}
...
/*
	...
	函数：int Boo33(void),void Input(struct List student[])
...
int Book33(void)
{
	...
	int i;
	int max = student1[0].grade;

	for (i = 0; i < 10; i++)
	{
		if (max< student1[i].grade)
		{
			printf("%s %d %d \n", student1[i].name, student1[i].data, student1[i].grade);
		}
	}
...
void Input(struct List student[])
{
	int i;
	for (i = 0; i < 5; i++)
	{
		...
		printf("请输入学生学号\n");
		scanf("%d", &student1[i].data);
		printf("请输入学生物理\n");
		scanf("%d", &student1[i].grade);
...
int Book34(void)
{
	...
	for (i = 0; i < 5; i++)
	{
		for (j = 0; j < 5-i; j++)
		{
			...
			if (student[i].data > student[i + 1].data)
			{
				temp = student[i].data;
				student[i].data = student[i + 1].data;
				student[i + 1].data = temp;
			}
	...
	for (i = 0; i < 1000; i++)
	{
		if (student[i].Class == 1)w++;
		if (student[i].Class == 2)s++;
		if (student[i].Class == 3)x++;
	}

	printf("一班:%d 二班:%d 三班:%d\n",w,s,x);
...
int Book35(void)
{
	...
	int day1[] = { 31,31,30,31,30,31,31,30,31,30 };
	int i,j,open=0,open2=0;

	for (i = 1; i <= 12; i++)
	{
		if (i <= date.month)open++;
	}
	...
	for (i = 1; i <= open; i++)
	{
		...
		{
			for (j = 0; j < 31; j++)open2++;
		}

		else if (((0 == date.years % 4) && (0 != date.years % 100)) || (0 == date.years % 400))
		{
			if (i == 2)
				{
					for (j = 0; j < 29; j++)open2++;
				}
		}

		else if (i == 2)
		{
			for (j = 0; j < 28; j++)open2++;
		}

		else if(i == 4 || i == 6 || i == 9 || i == 11)
		{
			for (j = 0; j < 30; j++)open2++;
		}
...
int Book36(void)
{
	...
	{
		printf("请输入图书编号\n");
		scanf("%d", &BookData[i].data);
		printf("请输图书名\n");
		scanf("%s", BookData[i].name);
		printf("请输入图书出版社\n");
		scanf("%s", BookData[i].name2);
		printf("请输入图书出版时间\n");
		scanf("%d", &BookData[i].date);
		printf("\n");
	}
	...
	for (i = 0; i < 5; i++)
	{
		if (Input == BookData[i].data)
		{
			if(BookData[i].open=='Y')printf("该书已借出\n");
			else
			{
				BookData[i].open = 'Y';
				printf("借阅成功%c\n", BookData[i].open);
...
int Book37(void)
{
	...
	struct  ListNode* head=Init_LinkList();
	For_LinkList(head);
...
void For_LinkList(struct  ListNode* head)
{
	if (head == NULL)return;

	struct  ListNode* For = head->next;
...
void Del_LinkList(struct  ListNode* head, int del)
{
